...
\subsection{Wie bin ich auf das Thema gekommen?} %TODO besser
...
Um die Problemfrage zu lösen werde ich zuerst ein Verfahren aussuchen. Daraufhin werde ich die gewählte Methode erklären und implementieren, um die Problemfrage zu beandworten. %TODO Zuletzt werde ich die Methode testen und wenn möglich verbessern.
...
Mit der Implementierung der Weierstraß-Iteration in Java ist eine Möglichkeit Nullstellen von Polynomen mithilfe des Computers zu approximieren gezeigt. Dabei ist jedoch wichtig zu erwähnen, dass es viele Unterschiedliche Verfahren mit unterschiedlichen Stärken und Schwächen gibt. Außerdem hat auch die Implementierung der einzelnen Methode einen großen Einfluss auf die Geschwindigkeit der Methode. In meiner Implementierung wurde mehr Fokus auf die Lesbarkeit des Codes, statt der Performance gelegt. Letzendlich können jedoch die Nullstellen der meisten Polynomfunktionen mithilfe dieser Implementierung gefunden werden.
