...
Bei der Implementierung der Weierstraß-Iteration in Java wurde größtenteils auf eine funktionale Programmierweise gesetzt. Dabei befinden sich in der Datei \glqq Weierstrass.java\grqq\space nur statische Funktionen. Neben der funktionalen Implementierung der Weierstraß-Iteration werden zwei Klassen verwendet. Einerseits sind die komplexen Zahlen und andererseits die Polynomfunktion in einer eigenen Klasse. Dabei wurden allerdings auch viele statische Hilfsfunktionen verwendet. Der \glqq Entry-Point\grqq\space der Weierstraß-Iteration befindet sich in der \glqq main\grqq -Methode der Klasse \glqq Weierstrass\grqq. Die weiteren \glqq main\grqq -Methoden sind nur zum Debugen.
...
Ein Array von Gleitkommazahlen (double), das die Koeffizienten des Polynoms enthält. Der Index repräsentiert den Exponenten des zugehörigen Terms.
\paragraph{degree}
Ein Integer, der den Grad des Polynoms angibt. Der Grad entspricht dem höchsten Exponenten im Polynom.
